...
class EncoderDecoder(nn.Module):
    ...
    def decode(self, memory, src_mask, tgt, tgt_mask, fc_feats):
        return self.decoder(self.tgt_embed(tgt), memory, src_mask, tgt_mask, fc_feats)
...
class DecoderLayer(nn.Module):
    ...
    def forward(self, x, memory, src_mask, tgt_mask, fc_feats):
        ...
        m = memory
        s = x
        x = torch.cat((fc_feats.unsqueeze(1), x), dim=1)
        # print(tgt_mask.size())
        # print(x.size())
        t = tgt_mask
        # print(tgt_mask.size())
